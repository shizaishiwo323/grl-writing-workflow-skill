...
1. `workflow_router`
2. `narrative_echo_and_payoff`
3. `mechanism_results`
4. `paragraph_engineering`
5. `sentence_hierarchy`
6. `ocar_funnel`
7. `methods_evidence_chain`
8. `story_architect`

Reason: `workflow_router` is the entrance; `narrative_echo_and_payoff` checks whether setup, payoff, and subsection continuity exist before local rewriting; `mechanism_results` and `paragraph_engineering` are the highest-value execution skills for the current manuscript; `sentence_hierarchy` should only be used after logic is stable.
...
| `narrative_echo_and_payoff` | Not yet separated; use `references/narrative_echo_and_payoff.md` until a dedicated prompt is created |
...
| Narrative Echo | `references/narrative_echo_and_payoff.md` | Promise-payoff map, forward pointers, backward links, thread summaries |
